...
start_dev = int(sys.argv[3])
end_dev =  int(sys.argv[4])
...
df_feat_b_dev.to_csv('df_feat_b_dev_{}-{}.csv'.format(start_train, end_train))
 
# Seizure Dataframe (features,target)
df_feat_s_dev = df_s.dropna()
df_feat_s_dev['Label'] = 'seiz'
df_feat_s_dev.to_csv('df_feat_s_dev_{}-{}.csv'.format(start_train, end_train))
 
# pre-Seizure Dataframe (features,target)
df_feat_pz_dev = df_pz.dropna()
df_feat_pz_dev['Label'] = 'pre-seiz'
df_feat_pz_dev.to_csv('df_feat_pz_dev_{}-{}.csv'.format(start_train, end_train))
